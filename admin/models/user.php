...
class HuruHelpdeskModelUser extends JModel
{
	var $_total = null;
	...
	function _buildQuery()
	{
		if(!$this->_query)
		{
			...
			$this->_query = "SELECT ju.id as j_id, ju.name, ju.username, ju.email, hh.id as id, hh.isuser, hh.isrep, hh.isadmin FROM #__users AS ju JOIN #__huruhelpdesk_users AS hh ON ju.id = hh.joomla_id";
			
			if($search != '')
			{
//				$fields = array('j_id','ju.name','ju.username','ju.email','id');
				$fields = array('ju.username','ju.name','ju.email');
				$where = array();
