...
<form action="index.php" method="post" name="adminForm">
	...
	<table class="adminlist">
		...
		for($i=0,$n=count($this->rows); $i<$n; $i++)
		{
			...
			$link = JFilterOutput::ampReplace('index.php?option=' . JRequest::getCmd('option') . '&view=category&task=edit&cid[]=' . $row->category_id);
			?>
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td align="center"> 
					<?php
					//get rep user info
					$name = userinfo($row->rep_id,'name');
					if(strlen($name)>0) echo $name;
